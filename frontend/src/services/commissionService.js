...
const commissionService = {
  ...
  getByProjectId: async (projectId) => {
    const response = await api.get(`/api/commissions/project/${projectId}`);
    return response.data;
  },

  // パートナーIDで手数料を取得
  getByPartnerId: async (partnerId) => {
    const response = await api.get(`/api/commissions/partner/${partnerId}`);
    return response.data;
  },

  // ステータスで手数料を取得
  getByStatus: async (status) => {
    const response = await api.get(`/api/commissions/status/${status}`);
    return response.data;
  },
  ...
  getTotalByPartnerId: async (partnerId) => {
    const response = await api.get(`/api/commissions/partner/${partnerId}/total`);
    return response.data;
  },

  // パートナーIDとステータスで手数料の合計金額を取得
  getTotalByPartnerIdAndStatus: async (partnerId, status) => {
    const response = await api.get(`/api/commissions/partner/${partnerId}/total/${status}`);
    return response.data;
  },
};
